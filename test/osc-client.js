...
const INVALID_OSC_MESSAGE = new Buffer(128);
...
describe("oscClient(client: net.Socket): net.Socket", function() {
  ...
  it("receive invalid message", (done) => {
    let outerReceiver = dgram.createSocket("udp4");
    let onMessageAtOuterReceiver = sinon.spy();
    let onMessageAtOscHubClient = sinon.spy();
    ...
    this.client.emit("data", INVALID_OSC_MESSAGE);

    setTimeout(() => {
      assert(onMessageAtOuterReceiver.callCount === 0);
      assert(onMessageAtOscHubClient.callCount === 0);
      assert(onErrorAtOscHubClient.callCount === 1);
      assert(onErrorAtOscHubClient.args[0][0].data === INVALID_OSC_MESSAGE);
  ...
  it("send invalid message", (done) => {
    ...
    this.client.oschubSockets[0].emit("message", INVALID_OSC_MESSAGE);

    setTimeout(() => {
      assert(this.client.write.callCount === 0);
      assert(onMessageAtOscHubClient.callCount === 0);
      assert(onErrorAtOscHubClient.callCount === 1);
      assert(onErrorAtOscHubClient.args[0][0].data === INVALID_OSC_MESSAGE);
